        ...
        <nav className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
                ...
                <Link to="/" className="flex items-center gap-2">
                    ...
                    <span className="font-heading text-xl font-bold text-maroon">EduReact</span>
                </Link>

                {/* Desktop links */}
                <div className="hidden md:flex items-center gap-6">
                    {navLinks.map((link)=>(
                        <a key={link.label} href={link.href} className="text-gray-700 hover:text-maroon transition-colors duration-200 text-sm font-medium">{link.label}
                        </a>
                    ))}
                </div>
                ...
                <div className="hidden md:flex items-center gap-3">
                    {user?(
                        <div>
                            ...
                            <button onClick = {handleLogout} className="flex items-center gap-1 text-sm bg-maroon/10 text-maroon px-3 py-1.5 rounded-lg hover:bg-maroon hover:text-white transition-colors duration-200">
